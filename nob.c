...
bool build_vulkan_app(Config config)
{
    bool result = true;
    Nob_Cmd cmd = {0};
    Nob_Procs procs = {0};
        ...
        case TARGET_LINUX: {
            if (config.hotreload) {
                ...
                cmd.count = 0;
                    nob_cmd_append(&cmd, "g++");
                    nob_cmd_append(&cmd, "-Wall", "-Wextra", "-ggdb");
                    nob_cmd_append(&cmd, "-o", "./build/vulkan_app");
                    nob_cmd_append(&cmd, "./main.cpp", "./vulkan_app.cpp");
                    nob_cmd_append(&cmd,
                        nob_temp_sprintf("-L./build/vulkan_utils/%s", NOB_ARRAY_GET(target_names, config.target)),
                        "-l:libraylib.a");
                    nob_cmd_append(&cmd, "-lglfw", "-lvulkan", "-lpthread", "-lX11", "-lXxf86vm", "-lXrandr", "-lXi");
...
static const char *vulkan_modules[] = {
    "vulkan_utils",
};

bool build_vulkan_utils(Config config)
{
    bool result = true;
    Nob_Cmd cmd = {0};
    Nob_File_Paths object_files = {0};

    if (!nob_mkdir_if_not_exists("./build/vulkan_utils")) {
        nob_return_defer(false);
    }

    Nob_Procs procs = {0};

    const char *build_path = nob_temp_sprintf("./build/vulkan_utils/%s", NOB_ARRAY_GET(target_names, config.target));

    if (!nob_mkdir_if_not_exists(build_path)) {
        nob_return_defer(false);
    }

    for (size_t i = 0; i < NOB_ARRAY_LEN(vulkan_modules); ++i) {
        const char *input_path = nob_temp_sprintf("./%s.cpp", vulkan_modules[i]);
        const char *output_path = nob_temp_sprintf("%s/%s.o", build_path, vulkan_modules[i]);
        switch (config.target) {
        case TARGET_LINUX:
        case TARGET_MACOS:
        case TARGET_WIN64_MINGW:
            output_path = nob_temp_sprintf("%s/%s.o", build_path, vulkan_modules[i]);
            break;
        case TARGET_WIN64_MSVC:
            output_path = nob_temp_sprintf("%s/%s.obj", build_path, vulkan_modules[i]);
            break;
        default: NOB_ASSERT(0 && "unreachable");
        }
        ...
        if (nob_needs_rebuild(output_path, &input_path, 1)) {
            ...
            switch (config.target) {
                ...
                case TARGET_MACOS:
                    ...
                    if(strcmp(vulkan_modules[i], "rglfw") == 0) {
        ...
        case TARGET_WIN64_MINGW: {
            if (!config.hotreload) {
                ...
                if (nob_needs_rebuild(libraylib_path, object_files.items, object_files.count)) {
                    ...
                    for (size_t i = 0; i < NOB_ARRAY_LEN(vulkan_modules); ++i) {
                        const char *input_path = nob_temp_sprintf("%s/%s.o", build_path, vulkan_modules[i]);
        ...
        case TARGET_WIN64_MSVC: {
            if (!config.hotreload) {
                ...
                if (nob_needs_rebuild(libraylib_path, object_files.items, object_files.count)) {
                    ...
                    for (size_t i = 0; i < NOB_ARRAY_LEN(vulkan_modules); ++i) {
                        const char *input_path = nob_temp_sprintf("%s/%s.obj", build_path, vulkan_modules[i]);
                        nob_cmd_append(&cmd, input_path);
                    }
...
bool build_dist(Config config)
{
    ...
    switch (config.target) {
        ...
        case TARGET_WIN64_MINGW: {
            ...
            if (!nob_copy_file("vulkan_app_logged.bat", "./musializer-win64-mingw/vulkan_app_logged.bat")) return 1;
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(subcommand, "build") == 0) {
        ...
        log_config(config);
        nob_log(NOB_INFO, "------------------------------");
        if (!build_vulkan_utils(config)) return 1;
        if (!build_vulkan_app(config)) return 1;
        if (config.target == TARGET_WIN64_MINGW || config.target == TARGET_WIN64_MSVC) {
            if (!nob_copy_file("musializer-logged.bat", "build/vulkan_app_logged.bat")) return 1;
        }
